...
func TestBoltStorageHandler_LastQueriedTxSlot(t *testing.T) {
	t.Parallel()

	dbPath, err := os.CreateTemp("", "solana-store-last-queried-slot-test-*")
	require.NoError(t, err)
	...
	slot, err := handler.GetLastQueriedTxSlot()
	require.NoError(t, err)
	require.Equal(t, uint64(0), slot)

	require.NoError(t, handler.SetLastQueriedTxSlot(479802399))

	slot, err = handler.GetLastQueriedTxSlot()
	require.NoError(t, err)
	require.Equal(t, uint64(479802399), slot)

	require.NoError(t, handler.SetLastQueriedTxSlot(479802450))

	slot, err = handler.GetLastQueriedTxSlot()
	require.NoError(t, err)
	require.Equal(t, uint64(479802450), slot)

	// the slot watermark must not disturb the unprocessed tx queue sharing the bucket
	sig, err := solana.NewWallet().PrivateKey.Sign([]byte("queued"))
	require.NoError(t, err)

	require.NoError(t, handler.PushUnprocessedTransactions([]solana.Signature{sig}))

	unprocessed, err := handler.GetAllUnprocessedTransactions()
	require.NoError(t, err)
	require.Equal(t, []solana.Signature{sig}, unprocessed)
}

func TestBoltStorageHandler_GetLatestEventSlot(t *testing.T) {
	t.Parallel()

	dbPath, err := os.CreateTemp("", "solana-store-latest-event-slot-test-*")
	require.NoError(t, err)
	...
	slot, err := handler.GetLatestEventSlot()
	require.NoError(t, err)
	require.Equal(t, uint64(0), slot)
	...
	for _, eventSlot := range []uint64{479806559, 479810737, 479814919} {
		sig, err := solana.NewWallet().PrivateKey.Sign([]byte{byte(eventSlot)})
		require.NoError(t, err)

		require.NoError(t, handler.StoreEvent(nil, eventSlot, 0, sig, programID,
			"BridgeRequestEvent", [32]byte{}, &testEvent{Value: 1}))
	}

	slot, err = handler.GetLatestEventSlot()
	require.NoError(t, err)
	require.Equal(t, uint64(479814919), slot)
}
